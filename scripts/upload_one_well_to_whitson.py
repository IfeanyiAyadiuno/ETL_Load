...
Requires whitson_connect.py in the repo root and Whitson credentials in .env:
  WHITSON_CLIENT, WHITSON_CLIENT_ID, WHITSON_CLIENT_SECRET, PROJECT_ID

Example:
  python scripts/upload_one_well_to_whitson.py --well-name "B2-01-85-26W6M"
  python scripts/upload_one_well_to_whitson.py --well-name "B2-01-85-26W6M" --start 2024-01-01 --end 2024-12-31
  python scripts/upload_one_well_to_whitson.py --well-name "B2-01-85-26W6M" --replace
"""
...
def _env(name: str, fallback: Optional[str] = None, required: bool = True) -> str:
    val = (os.getenv(name) or fallback or "").strip()
...
def main() -> int:
    ...
    parser.add_argument(
        "--project-id",
        default="",
        help="Whitson project id (default: PROJECT_ID or WHITSON_PROJECT_ID from .env)",
    )
    parser.add_argument(
        "--replace",
        action="store_true",
    ...
    client = _env("WHITSON_CLIENT")
    client_id = _env("WHITSON_CLIENT_ID")
    client_secret = _env("WHITSON_CLIENT_SECRET")
    project_raw = (args.project_id or "").strip() or _env(
        "WHITSON_PROJECT_ID", fallback=os.getenv("PROJECT_ID"), required=True
    )
