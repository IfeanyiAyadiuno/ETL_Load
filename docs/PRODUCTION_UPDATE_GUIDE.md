...
| **PCE_Production** | Replaces rows **only inside** the rolling window (~12 mo through today − 2); older daily rates are kept. Window insert writes daily rates only; then **all wells** get seq/cum recalculated from scratch (rolling-window wells from in-memory CDA, others from production; one bulk UPDATE via staging). | Full delete and rebuild from all CDA; sequences calculated from each well's first production before insert |
