...
def filter_to_first_production(df):
    """
    For each well, keep only rows from the first non-zero Gas WH Production onward
    """
    ...
    for well_idx, well_name in enumerate(wells, 1):
        ...
        # Find first row with non-zero Gas WH Production
        gas_wh = well_data['Gas WH Production (10³m³)'].fillna(0)
        non_zero_indices = gas_wh[gas_wh > 0].index
        ...
        if len(non_zero_indices) > 0:
            ...
            # No production data for this well
...
def calculate_sequences(df):
    ...
    for well_idx, well_name in enumerate(wells, 1):
        ...
        # Days Seq: simple counter starting at 1
        df.loc[well_indices, 'Days Seq'] = range(1, len(well_indices) + 1)
        
        # Day Seq UPRT: counter that repeats when Gas WH < 1
        ...
        while i < len(gas_wh):
            ...
            # If current value is >= 1, next day increments
            if gas_wh[i] >= 1:
                counter += 1
                i += 1
            else:
                # Current value < 1, check how many consecutive < 1
                j = i + 1
                while j < len(gas_wh) and gas_wh[j] < 1:
                    seq_uprt.append(counter)
                    j += 1
                # Move to next position after the streak
                i = j
                counter += 1
        ...
        # Show progress every 50 wells
        if well_idx % 50 == 0:
            print(f"    Processed {well_idx}/{total_wells} wells for sequences...")
    
    print("  Sequence calculations complete")
    return df
...
def calculate_sequences(df):
    """
    Calculate Days Seq and Day Seq UPRT for each well
    Day Seq UPRT stays the same during low production streaks
    """
    print("\nCalculating sequence columns...")
