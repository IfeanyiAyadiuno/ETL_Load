...
class ProdviewUpdateWorker(QThread):
    ...
    def on_checkbox_changed(self, row, state):
        ...
        if row < len(self.filtered_wells):
            ...
            # Only stage pending wells when checked
            if well in self.pending_wells and is_checked:
                # Check if already staged
                if well not in self.staged_wells:
                    self.staged_wells.append(well)
                    self.update_staged_table()
                    # Switch to Add New tab to show staged wells
                    self.tabs.setCurrentIndex(1)
                    self.status_label.setText(f"Staged {len(self.staged_wells)} well(s) for completion")
    
    def connect_composite_signals(self, row):
        """Connect signals to auto-update composite name"""
        widgets = self.row_widgets[row]
        
        def update_composite():
            """Update composite name based on other fields"""
            well_name = self.staged_wells[row].get('well_name', '')
            layer = widgets['dropdowns'].get('layer', None)
            tech = widgets['dropdowns'].get('completions_tech', None)
            orient = widgets['dropdowns'].get('orient', None)
            
            layer_text = layer.text() if layer and layer.text() else ""
            tech_text = tech.text() if tech and tech.text() else ""
            orient_text = orient.text() if orient and orient.text() else ""
            
            composite = WellMasterDB.compose_name(well_name, layer_text, tech_text, orient_text)
            if composite and widgets['composite']:
                widgets['composite'].setText(composite)
        
        # Connect each editable field's change signal
        for field, item in widgets['entries'].items():
            # For QTableWidgetItem, we need to connect to data changes
            # This is a bit tricky - we'll use a timer approach or connect to model signals
            pass
        
        for field, item in widgets['dropdowns'].items():
            # For QTableWidgetItem, we need to connect to data changes
            pass
        
        # For simplicity, we'll update composite when any cell is edited
        # This will be handled in the itemChanged signal
    ...
    def import_new_wells(self):
        ...
        for row in selected_rows:
            ...
            formation = self.staged_table.item(row, 4).text() if self.staged_table.item(row, 4) else None
            layer = self.staged_table.item(row, 5).text() if self.staged_table.item(row, 5) else None
            fault_block = self.staged_table.item(row, 6).text() if self.staged_table.item(row, 6) else None
            pad_name = self.staged_table.item(row, 7).text() if self.staged_table.item(row, 7) else None
            completions_tech = self.staged_table.item(row, 8).text() if self.staged_table.item(row, 8) else None
            lateral_length = self.staged_table.item(row, 9).text() if self.staged_table.item(row, 9) else None
            value_nav_uwi = self.staged_table.item(row, 10).text() if self.staged_table.item(row, 10) else None
            orient = self.staged_table.item(row, 11).text() if self.staged_table.item(row, 11) else None
            composite_name = self.staged_table.item(row, 12).text() if self.staged_table.item(row, 12) else None
            
            # Convert empty strings to None
            lateral_length = lateral_length if lateral_length and lateral_length.strip() else None
            ...
            update_data = {
                ...
                'fault_block': fault_block if fault_block else None,
                'pad_name': pad_name if pad_name else None,
                'completions_tech': completions_tech if completions_tech else None,
                'lateral_length': float(lateral_length) if lateral_length else None,
                'value_nav_uwi': value_nav_uwi if value_nav_uwi else None,
                'orient': orient if orient else None,
                'composite_name': composite_name if composite_name else None
            }
    ...
    def remove_from_staging(self):
        ...
        QMessageBox.information(self, "Remove", "Remove functionality coming soon")
